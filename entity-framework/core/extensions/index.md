...
LLBLGen Pro is an entity modeling solution with support for Entity Framework and Entity Framework Core. It lets you easily define your entity model and map it to your database, using database first or model first, so you can get started writing queries right away. For EF Core: 6, 7.

[Website](https://www.llblgen.com/)

### Devart Entity Developer

Entity Developer is a powerful O/RM designer for ADO.NET Entity Framework, NHibernate, LinqConnect, Telerik Data Access, and LINQ to SQL. It supports designing EF Core models visually, using model first or database first approaches, and C# or Visual Basic code generation. For EF Core: 6, 7.
...
A plugin library that enables automatically recording the data changes performed by EF Core into a history table. For EF Core: 6.
...
Second level caching is a query cache. The results of EF commands will be stored in the cache, so that the same EF commands will retrieve their data from the cache rather than executing them against the database again. For EF Core: 6, 7.
...
Allows customization of classes reverse engineered from an existing database using the Entity Framework Core tool chain with Handlebars templates. For EF Core: 6, 7.
...
NeinLinq extends LINQ providers such as Entity Framework to enable reusing functions, rewriting queries, and building dynamic queries using translatable predicates and selectors. For EF Core: 6, 7.
...
EF Core extensions for Bulk operations (Insert, Update, Delete). For EF Core: 6, 7.
...
Adds design-time pluralization. For EF Core: 6, 7.
...
Extends [Verify](https://github.com/VerifyTests/Verify) to allow snapshot testing with Entity Framework. For EF Core: 6, 7.
...
Provides a wrapper around [SQL Server Express LocalDB](/sql/database-engine/configure-windows/sql-server-express-localdb) to simplify running tests against Entity Framework. For EF Core: 6, 7.

[GitHub repository](https://github.com/SimonCropp/LocalDb) | [NuGet](https://www.nuget.org/packages/EfLocalDb)

### EntityFrameworkCore.Projectables

Flexible projection magic for EF Core. Use properties, methods, and extension methods in your query without client evaluation. For EF Core: 6.
...
Triggers for EF Core. Respond to changes in your DbContext before and after they are committed to the database. Triggers are fully asynchronous and support dependency injection, inheritance, cascading and more. For EF Core: 6.
...
Extends your DbContext with features such as: Include Filter, Auditing, Caching, Query Future, Batch Delete, Batch Update, and more. For EF Core: 6, 7.
...
Extends your DbContext with high-performance bulk operations: BulkSaveChanges, BulkInsert, BulkUpdate, BulkDelete, BulkMerge, and more. For EF Core: 6, 7.

[Website](https://entityframework-extensions.net/) | [NuGet](https://www.nuget.org/packages/Z.EntityFramework.Plus.EFCore)

### Expressionify

Add support for calling extension methods in LINQ lambdas. For EF Core: 6.

[GitHub repository](https://github.com/ClaveConsulting/Expressionify) | [NuGet](https://www.nuget.org/packages/Clave.Expressionify)

### EntityLinq

Alternative (not MS based) Language Integrated Query (LINQ) technology for relational databases. It allows you to use C# to write strongly typed SQL queries. For EF Core: 6.
...
This will automatically make all your table and column names have snake_case, all UPPER or all lower case naming. For EF Core: 6, 7.
...
This plugin allows you to opt into some check constraints - just activate it and they'll automatically get created for you. For EF Core: 6, 7.
...
Adds native support to EntityFrameworkCore for SQL Server for the NodaTime types. For EF Core: 6, 7.
...
Adds hierarchyid support to the SQL Server EF Core provider. For EF Core: 6, 7.
...
Alternative translator of LINQ queries to SQL expressions. For EF Core: 6, 7.
...
An implementation for soft deleting entities. For EF Core: 6.
...
Extends EF Core to resolve connection strings from App.config. For EF Core: 6, 7.
...
A DTO-Entity mapper with composition/aggregation handling (similar to GraphDiff). For EF Core: 6.
...
Supports SQL Server, Postgres, MySql, SQLite and Oracle. For EF Core: 6.

[GitHub Repository](https://github.com/Giorgi/EntityFramework.Exceptions)

### EntityFrameworkCore.FSharp

Adds F# design-time support to EF Core. For EF Core: 6.
...
Adds VB design-time support to EF Core. For EF Core: 6, 7.
...
Extension for Entity Framework that allows you to create and manage multiple query filters. For EF Core: 6, 7.

[GitHub repository](https://github.com/Krzysztofz01/EFCore.QueryFilterBuilder)

### Laraue.EfCoreTriggers

Fluent API to declare triggers in `Context.OnModelCreating` which are later built into migrations. Providers to Postgres, MySQL, SQL Server and SQLite. For EF Core: 6, 7.
...
Cloning entities using EF Core configuration. You can use the `Include` method to specify related data to be cloned. For EF Core: 6, 7.
...
The Dynamic LINQ library let you execute query with dynamic string and provide some utilities methods such as ParseLambda, Parse, and CreateClass. For EF Core: 6, 7.
...
GraphQL server with tight EntityFramework integration. For EF Core: 6, 7.
