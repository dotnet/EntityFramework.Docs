...
When using Visual Studio, we recommend the [Package Manager Console tools](xref:core/miscellaneous/cli/powershell) in lieu of the CLI tools. Package Manager Console tools automatically:

* Work with the current project selected in the **Package Manager Console** without requiring that you manually switch directories.
* Open files generated by a command after the command is completed.
