...
We frequently receive requests to adjust the code generated when scaffolding (reverse engineering) from an existing database. We plan to address these requests in EF7 by supporting [T4 templates](/visualstudio/modeling/code-generation-and-t4-text-templates?view=vs-2022) for the generated entity types and `DbContext`. Developers will be able to customize the standard templates, or create new templates from scratch.
...
- [Windows Forms](/dotnet/desktop/winforms/overview/?view=netdesktop-6.0)
