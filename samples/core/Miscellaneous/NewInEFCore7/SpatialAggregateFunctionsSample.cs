        ...
        where TContext : GeoCacheContext, new()
    {
        await using (var context = new TContext())
        {
            ...
            await context.AddRangeAsync(
                new GeoCache { Name = "Sandpiper Cache", Owner = "Nilwob Inc.", Location = new Point(-93.71855, 41.760783) { SRID = 4326 } },
                new GeoCache { Name = "Paddy's Pot-O-Gold", Owner = "Nilwob Inc.", Location = new Point(-93.733633, 41.775633) { SRID = 4326 } },
                new GeoCache { Name = "Blazing Beacon", Owner = "The Spokes", Location = new Point(-1.606483, 55.392433) { SRID = 4326 } },
                new GeoCache { Name = "133 Steps to Relaxation", Owner = "Team isuforester", Location = new Point(-93.854733, 41.9389) { SRID = 4326 } },
        ...
        await using (var context = new TContext())
        {
            var query = context.Caches
                ...
                .Select(grouping => new
                    {
                        Id = grouping.Key,
                        ConvexHull = NetTopologySuite.Algorithm.ConvexHull.Create(grouping.Select(cache => cache.Location))
                    });
        ...
        await using (var context = new TContext())
        {
            var query = context.Caches
                ...
                {
                    Id = grouping.Key,
                    Union = UnaryUnionOp.Union(grouping.Select(cache => cache.Location))
                });
