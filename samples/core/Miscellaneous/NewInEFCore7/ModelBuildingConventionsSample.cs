...
public static class ModelBuildingConventionsSample
{
    ...
    public static async Task Custom_model_validation_convention()
    {
        PrintSampleName();

        await using var context = new TenantIdValidatingContext();
        await context.Database.EnsureDeletedAsync();
        await context.Database.EnsureCreatedAsync();
        Console.WriteLine(context.Model.ToDebugString());
        Console.WriteLine();
...
{
    protected ModelBuildingBlogsContextBase(bool useSqlite = false)
        : base(useSqlite)
    {
    }
...
public class LaundryContext : DbContext
{
    ...
    protected override void ConfigureConventions(ModelConfigurationBuilder configurationBuilder) => configurationBuilder.Conventions.Replace<PropertyDiscoveryConvention>(
...
public class TenantIdValidatingContext : DbContext
{
    public DbSet<LaundryBasket> LaundryBaskets => Set<LaundryBasket>();
...
public class NoCascadeDeleteConvention : IModelFinalizingConvention
{
    public void ProcessModelFinalizing(IConventionModelBuilder modelBuilder, IConventionContext<IConventionModelBuilder> context)
    {
        foreach (IConventionForeignKey? foreignKey in modelBuilder.Metadata.GetEntityTypes()
