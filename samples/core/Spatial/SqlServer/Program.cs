...
Country currentCountry = db.Countries
    .FirstOrDefault(c => c.Border.Contains(currentLocation));
#endregion
Console.WriteLine($"Current country: {currentCountry.CountryName}");

// Find which states/provinces a route intersects
LineString route = new GeoJsonReader().Read<LineString>(File.ReadAllText("seattle-to-new-york.json"));
route.SRID = 4326;
...
foreach (StateProvince state in statePorvincesIntersected)
{
